#!/bin/bash
#
# Gemini-Centric Orchestrator v1.0
# Gemini-CLI를 중심으로 Claude Code, Codex가 협업하는 오케스트레이션
#
# 개선사항:
# - Gemini 중심 워크플로우로 재편
...
print_logo() {
    echo -e "${PURPLE}"
    echo "╔══════════════════════════════════════════════════════════╗"
    echo "║          Gemini-Centric Orchestrator v1.0                ║"
    echo "║      Gemini CLI | Claude Code | OpenAI Codex            ║"
    echo "╚══════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
# 도움말 출력
print_help() {
    echo -e "${GREEN}Usage:${NC} ./orchestrator_gemini.sh <command> [options]"
    echo ""
    ...
    echo ""
    echo -e "${YELLOW}Workflows (Gemini-Centric):${NC}"
    echo "  feature    새 기능 개발 (gemini → claude → gemini → codex)"
    echo "  review     멀티 에이전트 코드 리뷰 (parallel)"
    echo "  bugfix     버그 분석 및 수정 (gemini → codex → claude)"
    echo "  refactor   코드 리팩토링 (gemini → claude → codex)"
    echo "  docs       문서화 (parallel: gemini + claude)"
    ...
    echo "  ./orchestrator_gemini.sh run feature -p \"사용자 인증 기능 추가\""
    echo "  ./orchestrator_gemini.sh parallel -p \"이 코드를 리뷰해줘\" -f ./src/main.py"
    echo "  ./orchestrator_gemini.sh ask gemini -p \"아키텍처 설계를 도와줘\""
    echo "  ./orchestrator_gemini.sh compare -p \"버블 정렬 구현\" -v"
    echo ""
...
# 워크플로우 목록
list_workflows() {
    log_info "사용 가능한 워크플로우 (Gemini-Centric):"
    echo ""
    echo -e "  ${CYAN}feature${NC}     새 기능 개발"
    echo -e "              └─ ${PURPLE}pipeline:${NC} gemini(설계) → claude(리뷰) → gemini(구현) → codex(테스트)"
    echo ""
    echo -e "  ${CYAN}review${NC}      코드 리뷰"
    echo -e "              └─ ${PURPLE}parallel:${NC} gemini + claude + codex (동시 실행)"
    echo ""
    echo -e "  ${CYAN}bugfix${NC}      버그 수정"
    echo -e "              └─ ${PURPLE}role-based:${NC} gemini(분석) → codex(수정) → claude(검증)"
    echo ""
    echo -e "  ${CYAN}refactor${NC}    리팩토링"
    echo -e "              └─ ${PURPLE}pipeline:${NC} gemini(분석) → claude(수정) → codex(검증)"
    echo ""
    echo -e "  ${CYAN}docs${NC}        문서화"
    echo -e "              └─ ${PURPLE}parallel:${NC} gemini(초안) + claude(보완)"
    echo ""
...
run_workflow() {
    ...
    case "$workflow" in
        feature)
            echo -e "${PURPLE}[WORKFLOW]${NC} 새 기능 개발 워크플로우 (Gemini-Centric)"
            "$SCRIPTS_DIR/workflow_feature_gemini.sh" "$prompt" "$file" "$custom_output"
            ;;
        review)
            ...
            ;;
        bugfix)
            echo -e "${PURPLE}[WORKFLOW]${NC} 버그 수정 워크플로우 (Gemini-Centric)"
            "$SCRIPTS_DIR/workflow_bugfix_gemini.sh" "$prompt" "$file" "$custom_output"
            ;;
        refactor)
            echo -e "${PURPLE}[WORKFLOW]${NC} 리팩토링 워크플로우 (Gemini-Centric)"
            run_pipeline "$prompt" "$file" "$custom_output" "gemini" "claude" "codex"
            ;;
        docs)
    ...
    case "$command" in
        run)
            if [ -z "$workflow_name" ]; then
                ...
                echo "사용법: ./orchestrator_gemini.sh run <workflow> -p <prompt>"
                exit 1
            fi
            if [ -z "$prompt" ]; then
        ...
        ask)
            if [ -z "$agent_name" ]; then
                ...
                echo "사용법: ./orchestrator_gemini.sh ask <agent> -p <prompt>"
